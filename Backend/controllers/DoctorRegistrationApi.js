  ...
  Docobj.save()
    .then((inserteddocument1) => {
      mailservice.sendmail(
        ...
        "THANK YOU FOR REGISTRATION" + inserteddocument1
      );
      res
        .status(200)
        .send(
          "DOCUMENT INSERED IN MONGODB DATABASE" + "<br>" + inserteddocument1
        );
    }) //CLOSE THEN
    ...
    DocModel.find({
      ...
      .catch((err) => {
        return res
          ...
          .send({
            message:
              "DB Problem..Error in Retriving with id " + req.params.Doctorid,
          });
      }); //CLOSE CATCH
  } //CLOSE CALLBACK FUNCTION BODY
); //CLOSE POST METHOD
    ...
    DocModel.findOneAndRemove({ DoctorEmail: req.params.eid })
      ...
      .catch((err) => {
        return res
          .status(500)
          .send({
            message: "DB Problem..Error in Delete with id " + req.params.eid,
          });
      }); //CLOSE CATCH
    ...
    DocModel.find({ DoctorEmail: req.params.eid })
      .then((getsearchdocument) => {
        if (getsearchdocument.length > 0) {
          ...
          return res
            .status(404)
            .send({ message: "Note not found with id " + req.params.eid });
        }
      }) //CLOSE THEN
      .catch((err) => {
        return res
          .status(500)
          .send({
            message: "DB Problem..Error in Retriving with id " + req.params.eid,
          });
      }); //CLOSE CATCH
  } //CLOSE CALLBACK FUNCTION BODY
); //CLOSE GET METHOD
